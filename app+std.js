;(function() {
var ui_anchor_anchor, demo_modal_index, demo_tab_index, demo_message_index, demo_autoComplete_index, demo_toast_index, demo_getstarted_index, demo_tooltip_index, demo_sidenav_index, demo_dateutil_index, demo_tree_index, demo_rate_index, demo_progress_index, demo_calendar_index, demo_dropdown_index, demo_ripple_index, demo_checkbox_index, demo_switcher_index, demo_button_index, demo_radio_index, demo_loading_index, demo_pagination_index, demo_accordion_index, demo_validation_index, bootstrap;
(function ($) {
  var Anchor = function (target, settings) {
      var current, mappings = {};
      target.find(settings.selector4anchor + '[' + settings.symbol + ']').filter('.ui.anchor [' + settings.symbol + ']').each(function () {
...
demo_dateutil_index = function () {
  angular.module('demo.dateutil', []).controller('dateutilController', [
    '$scope',
    function ($scope) {
      var now = new Date(), justAgo = new Date(now - 50 * 1000), minuteAgo = new Date(now - 200 * 1000), hourAgo = new Date(now - 3600 * 1000), yesterday = new Date($.dateutil(now).yesterday()), morethan = new Date($.dateutil(now).day(-31));
      $scope.init = function () {
        $.anchor();
      };
      angular.extend($scope, {
    ...
    function ($scope) {
      ...
      $scope.afterInit = function (szseTree) {
        $.when(deferred).done(function () {
          szseTree.expand('szse').expand('zhyjs').disabled('zhyjs').disabled('szzqjysyyb');
        });
      };
    }
  ]);
